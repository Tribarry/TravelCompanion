...
(() => {
  ...
  const CONTEXT = {
    ...
    'Cát Tiên National Park': 'Cát Tiên is lowland rainforest on the Đồng Nai river system, about four hours north of Saigon. It is one of the last large southern forests with yellow-cheeked gibbons, Siamese crocodiles, hornbills and a working primate-rescue island — a real jungle overnight, not a city day trip.',
  ...
  function applySection(d,sections){
    ...
    d.experiences=items.map(toExperience); d.context=CONTEXT[d.name]||CONTEXT[key]||d.summary; d.summary=d.context;
    {
      ...
      if(/ho chi minh/.test(destN)){
        ...
        d.experiences.forEach(e=>{const hit=HCMC.find(([rx])=>rx.test(e.name||''));if(hit)e.photo=hit[1]});
      }
    }
  ...
  function fixCoordsAndWiki(){
    ...
    const coords={
      'Bến Tre':[10.2434,106.3756],'Trà Vinh':[9.9347,106.3453],'Nam Du Islands':[9.681,104.354],'Vũng Tàu':[10.4114,107.1362],'Cát Tiên National Park':[11.4225,107.4283],'Hang Én':[17.48,106.29],'Pygmy / Hung Thoong':[17.45,106.25],'Sơn Đoòng':[17.456,106.287],'Hà Nội Return':[21.0285,105.8542]
    };
    const wiki={'Bến Tre':'Bến Tre','Trà Vinh':'Trà Vinh','Nam Du Islands':'Nam Du','Vũng Tàu':'Vũng Tàu','Cát Tiên National Park':'Cát Tiên National Park','Hang Én':'Hang Én','Pygmy / Hung Thoong':'Phong Nha-Kẻ Bàng National Park','Sơn Đoòng':'Sơn Đoòng Cave','Hà Nội Return':'Hanoi'};
  ...
  const READY=(async()=>{
    ...
    insertDestination(list,'Cát Tiên National Park','Vũng Tàu','Cát Tiên National Park',sections,'1–2');
